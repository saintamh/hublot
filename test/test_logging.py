...
def test_basic_logging(client, server, captured_logs):
    ...
    assert re.search(fr'^\[\w\w\w/\w{{13}}\] \[{engine}\]     {server}/hello\n$', captured_logs())
    client.get(f'{server}/hello')
...
def test_logging_courtesy_sleep(client, server, captured_logs):
    ...
    assert re.search(fr'^\[\w\w\w/\w{{13}}\] \[{engine}\]     {server}/echo\?x=1\n$', captured_logs())
    client.get(f'{server}/echo?x=2')
    assert re.search(fr'^\[\w\w\w/\w{{13}}\] \[{engine}\+5s\]  {server}/echo\?x=2\n$', captured_logs())
...
def test_logging_redirects(client, server, captured_logs):
    ...
    assert re.search(
        r'^'
        fr'\[\w\w\w/\w{{13}}\] \[{engine}\]     {server}/redirect/chain/1\n'
        fr'\[\w\w\w/\w{{13}}\] \[{engine}\]     -> {server}/redirect/chain/2\n'
        fr'\[\w\w\w/\w{{13}}\] \[{engine}\]     -> {server}/redirect/chain/3\n'
        r'$',
        captured_logs(),
    )
...
def test_logging_on_retry(client, server, unique_key, captured_logs):
    ...
    assert re.search(
        r'^'
        fr'\[\w\w\w/\w{{13}}\] \[{engine}\]     {server}/fail-twice-then-succeed/{unique_key}\n'
